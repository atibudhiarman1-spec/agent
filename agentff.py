...
class State(TypedDict, total=False):
    ...
    mapping: Dict              # loaded YAML/JSON
    df: pd.DataFrame
    tempdir: str

    chart_plan: dict           # ChartPlan as dict (from LLM + requirements overlay)
    charts: List[Dict[str, object]]  # prepared chart payloads (native + PNG fallback)
    insights: List[str]
    ...
    Expected keys (all OPTIONAL):
      ...
      requirements:
        ...
        <chart_id>:
          ...
          agg: "sum"|"mean"|"none"
          top_k: <int>                            # for bar
      place_unmapped: true|false   # optional; default False when mapping exists, True when no mapping
    """
    if not path:
        return {}
...
def plan_charts_with_gemini(s: State) -> State:
    ...
    try:
        ...
        df = s["df"]
        charts = []
        dt = _pick_datetime(df); num = _pick_numeric(df)
        if dt and num:
    ...
    for cid, req in reqs.items():
        ...
        if isinstance(req, str):
            ...
            if req_type not in ("line", "bar"):
                ...
                continue
            # synthesize columns if needed
            if req_type == "line":
                x = _pick_datetime(df); y = _pick_numeric(df)
                if not x or not y:
...
def render_charts_prepare_payloads(s: State) -> State:
    ...
    for spec in plan.charts:
        if spec.type == "line":
            ...
            fig.savefig(img, dpi=200); plt.close(fig)
            ...
            grp = dfx.groupby("x")["y"]
            if spec.agg == "mean": grp = grp.mean()
            else: grp = grp.sum()
            grp = grp.sort_values(ascending=False)
            if spec.top_k: grp = grp.head(spec.top_k)
            dfx = grp.reset_index()
            ...
            fig = plt.figure()
            plt.bar(categories, values); plt.xticks(rotation=45, ha="right")
            plt.title(title); plt.tight_layout()
...
def generate_insights_gemini(s: State) -> State:
    df = s["df"]; charts = s.get("charts", [])
    log = s.get("log", [])

    # tiny aggregates to keep tokens low
    ...
    prompt = (
        "Produce a concise, business-friendly executive summary (1–2 sentences) plus 3–5 bullet insights. "
...
# ------- PPT Binder: native editable charts, mapped shapes, auto-add slides -------
...
def bind_ppt_native_with_mapping_and_fallback(s: State) -> State:
    ...
    # New: control whether to auto-place charts that weren't mapped
    # Default: if mapping exists, DON'T auto-place leftovers; if no mapping, DO auto-place.
    ...
    # 1) First, honor mappings by placing into named shapes across the existing template
    for cid, shape_name in mapped.items():
        ...
        if slide_found and shp:
            ...
            try:
                ...
                placed = False
            # b) if placeholder, try insert_chart
            if not placed:
                try:
                    ...
                    placed = False
            # c) otherwise, add a new chart at the same bounds
            if not placed:
                try:
                    ...
                    placed = False
            # d) last resort: drop PNG at same bounds
            if not placed:
                try:
    ...
    # 2) Auto-place any charts not yet used (only if allowed)
    ...
    # 3) Insights — write into a named shape if present anywhere; else a new slide
    tgt_slide, tgt_shape = _find_shape_anywhere(prs, insights_target)
    if tgt_shape is not None and hasattr(tgt_shape, "text_frame"):
        tf = tgt_shape.text_frame; tf.clear()
        for i, bullet in enumerate(s.get("insights", []) or []):
            p = tf.add_paragraph() if i > 0 else tf.paragraphs[0]
            p.text = bullet; p.level = 0; p.font.size = Pt(14); p.alignment = PP_ALIGN.LEFT
    else:
        slide = prs.slides.add_slide(prs.slide_layouts[title_layout_idx])
        try:
...
def build_app():
    ...
    g.add_node("plan_charts", plan_charts_with_gemini)       # optional (skipped if requirements cover everything)
    g.add_node("enforce_requirements", enforce_requirements_overlay)  # overlay/override
